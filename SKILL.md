...
- 각 문항은 `companies/<company-name>/questions/q1.md`, `q2.md` 같은 개별 파일에서 작업한다.
- 각 문항 파일에는 `## 질문 태그` 섹션을 두고, `question-library/`의 분류 체계를 따라 `대표 질문 유형`과 `보조 태그`를 먼저 적는다.
- 각 문항 파일에는 `## 유사 문항 참조` 섹션을 두고, 질문이 비슷한 기존 문항 파일을 `companies/*/questions/*.md`에서 먼저 찾은 뒤 경로와 활용 이유를 남긴다.
...
- 각 문항은 내부 내용 루프를 끝낸 뒤 `scripts/validation.py`로 마지막 검증을 하고, 그 다음 CLI에 `초안`, `리뷰어 평가`, `수정 반영`, `최종 답변`을 보여준 뒤 사용자 피드백을 받는다.
...
- 초안 작성 전에는 `## 참조 자산` 섹션에 실제로 읽은 파일 경로를 남긴다. 최소한 `assets/resume.md`, `assets/portfolio.md`, `assets/credentials.md`, `companies/<company-name>/company.md`, `companies/<company-name>/references/` 아래 참고자료 1개 이상을 적어야 한다.
- 현재 회사 폴더에 기존 질문 파일이나 `before`, `after`, `comparison` 같은 작업 이력이 있으면, `companies/<company-name>/questions/` 아래 파일도 최소 1개 이상 반드시 참조 자산에 포함한다.
- 질문이 유사하면 현재 회사 여부와 관계없이 `companies/*/questions/` 아래 기존 문항을 먼저 참조한다. 내 기존 문항은 합격 자소서보다 앞선 1차 기준 자산으로 취급한다.
- 유사 문항 판단은 기본적으로 `question-library/`의 `대표 질문 유형`을 먼저 보고, 그다음 `보조 태그`, 질문 원문, 경험 배치가 얼마나 비슷한지로 좁힌다.
